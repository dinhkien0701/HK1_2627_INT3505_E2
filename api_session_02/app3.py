...
def put(bid):
    ...
    if not t or not a:
        return jsonify("need title + author"), 422
...
# Nâng cấp GET/books thành một sản phẩm sẵn sàng để dùng

# app3.py voi pagnination + filter + HATEOAS
...
def list_books():
    try:
        ...
        return jsonify(error = "page and size must be int")
    page = max(page, 1)
    size = max(min(size, MAX_SIZE), 1)

    #filter : author chinh xac , tim q trong title
    flt = BOOKS
